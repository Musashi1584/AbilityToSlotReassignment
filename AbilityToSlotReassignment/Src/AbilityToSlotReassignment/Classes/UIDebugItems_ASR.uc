//-----------------------------------------------------------
//	Class:	UIDebugItems_ASR
//	Author: Musashi
//	
//-----------------------------------------------------------


class UIDebugItems_ASR extends UIDebugItems;
...
simulated function PopulateItemDropdown(UIDropdown kDropdown, EInventorySlot eEquipmentType)
{
	...
	foreach class'X2ItemTemplateManager'.static.GetItemTemplateManager().IterateTemplates(kEquipmentTemplate, none)
	{
		if( (X2EquipmentTemplate(kEquipmentTemplate) != none &&
			...
			class'Helper'.static.TriggerOverrideShowEquipmentTemplateInUI(EInventorySlot(TypeDropdown.SelectedItem), X2EquipmentTemplate(kEquipmentTemplate))
		)
		{
...
{
	class'X2DownloadableContentInfo_AbilityToSlotReassignment'.static.ASR_GiveItem(ItemDropdownR.GetSelectedItemText(), EInventorySlot(TypeDropdown.SelectedItem));
