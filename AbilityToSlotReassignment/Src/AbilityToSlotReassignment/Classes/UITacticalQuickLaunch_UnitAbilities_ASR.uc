//-----------------------------------------------------------
//	Class:	UITacticalQuickLaunch_UnitAbilities_ASR
//	Author: Musashi
//	
//-----------------------------------------------------------


class UITacticalQuickLaunch_UnitAbilities_ASR extends UITacticalQuickLaunch_UnitAbilities config(GameData);
...
simulated function InitAbilities(UITacticalQuickLaunch_UnitSlot Slot)
{
	...
	AWCAbilities = UITacticalQuickLaunch_UnitSlot_ASR(Slot).m_AWCAbilities;
...
simulated function BuildSoldierAbilities()
{
	...
	m_ypos = 0;
	AbilityTree = OriginatingSlot.m_arrAbilityTree;
	for (j = 0; j < AbilityTree.Length; ++j)
	{
		if (AbilityTree[j].AbilityName != '')
		{
			...
			{
				if (Progression.iRank == 0 && Progression.iBranch == j)
				{
...
simulated function SaveButton(UIButton kButton)
{
	...
	UITacticalQuickLaunch_UnitSlot_ASR(OriginatingSlot).m_AWCAbilities = AWCAbilities;
